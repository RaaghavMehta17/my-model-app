...
export default class EthereumRpc {
  ...
  async signMessage() {
    try {
      ...
      // const signer = ethersProvider.getSigner();
      const signer = await ethersProvider.getSigner();
      const originalMessage = "Hey! Please sign the message";
  ...
  async deployContract(): Promise<any> {
    ...
    const contractFactory = new ContractFactory(JSON.parse(JSON.stringify(contractABI), contractByteCode, signer));
